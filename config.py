...
WATCH_ITEMS = [
    ...
    # nyuka-nowのクラウドIP遮断は2026-07-10に解除を確認し、Actionsで本番稼働中。
    # 再遮断されてもサーキットブレーカーで自動吸収（判定不能=前回維持）される。
    # anime-matsuriに専用ページが無い領域（DBFW横断・遊戯王横断・コナン）をカバーする。
    {
...
# カード以外の商品カテゴリタグ（nyuka-now等の集約ページの行頭タグ）。対象はカード商品のみ
# のため、これらを含む行は原則通知しない（実例: 【雑誌】週刊少年ジャンプが毎週通知されかけた）。
# 【例外】雑誌・書籍でも「カード付録」（Vジャンプのプロモカード等）は購入対象になり得るため、
# カード付録を示す語（MAGAZINE_CARD_MARKERS）を含む場合は通知する（ユーザー指示 2026-07-14）。
NON_CARD_CATEGORY_TAGS = [
    "【雑誌】", "【書籍】", "【コミック】", "【フィギュア】", "【ぬいぐるみ】",
    "【CD】", "【DVD", "【Blu-ray", "【ゲーム】", "【プラモデル】", "【ホビー】",
]
