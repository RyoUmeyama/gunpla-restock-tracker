...
WATCH_ITEMS = [
    ...
    {
        # 厳格モード: ニュース一覧のため「発売告知」だけの行は通知せず、
        # 抽選/再販/受付等の行動語がある行だけ通知する（strict_actions）。
...
# 転売妙味がなく通知価値のない定番商品カテゴリ（いつでも定価で買える＝チャンスではない）。
# これを含む行は差分通知・ダイジェストから除外する。
# ※「カードセット」は30th CELEBRATIONカードセット等のプレミア商品があるため除外しない。
NOISE_PRODUCT_KEYWORDS = [
    "スターターセット", "構築デッキ", "デッキビルド", "スタートデッキ",
